...
class FavHotelPage extends StatefulWidget {
  const FavHotelPage({Key? key}) : super(key: key);

  @override
  State<FavHotelPage> createState() => _FavHotelPageState();
}

class _FavHotelPageState extends State<FavHotelPage> {
    ...
    return BackgroundPage(
      ...
      child: Column(
        children: [
          ...
          Expanded(
            child: BlocConsumer<FavouriteBloc, FavState>(
              bloc: _bloc,
              ...
              builder: (context, state) {
                if (state is LoadingGetFavHotelsState) {
                  return const AppLoader();
                } else if (state is ErrorGetFavHotelState) {
                ...
                if ((state as SuccessGetFavHotelState).hotels.isEmpty) {
                  return Center(
                      child:
                ...
                return SmartRefresher(
                    ...
                    child: SingleChildScrollView(
                      child: Column(
                        children: List.generate((state).hotels.length, (i) {
                          return HotelCard(
                            model: (state).hotels[i],
                            getRefresh: () {
